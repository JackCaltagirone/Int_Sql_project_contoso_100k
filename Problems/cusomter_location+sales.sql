...
SELECT s.orderdate,
    ...
    ROUND(
        SUM(
            CASE
                WHEN c.continent = 'Europe' THEN (s.quantity * s.netprice * s.exchangerate)::numeric
                ELSE 0
            END
        ),
        2
    ...
    ROUND(
        SUM(
            CASE
                WHEN c.continent = 'Australia' THEN (s.quantity * s.netprice * s.exchangerate)::numeric
                ELSE 0
            END
        ),
        2
    ...
    ROUND(
        SUM(
            CASE
                WHEN c.continent = 'North America' THEN (s.quantity * s.netprice * s.exchangerate)::numeric
                ELSE 0
            END
        ),
        2
    ) AS north_america_sales
FROM sales AS s -- Join customer table to access continent information
    LEFT JOIN customer AS c ON s.customerkey = c.customerkey -- Filter for 2023 only
WHERE s.orderdate >= '2023-01-01'
    AND s.orderdate < '2023-12-31' -- Aggregate by day
GROUP BY s.orderdate -- Sort chronologically
